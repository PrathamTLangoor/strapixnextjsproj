...
const EmployeeCard = ({ employees, managers }) => {
    return (
        <div className='flex flex-row gap-4 pl-3 mt-5 flex-wrap'>
            {employees &&
                employees.data.map((employee) => {
                    ...
                    return (
                        <Link href={"/employee/[employeeId]"} as={`/employee/${employee.attributes.employee_id}`} key={employee.id} className='flex gap-4 justify-evenly items-center bg-black text-white focus:bg-black focus:text-white hover:bg-white border-2 border-[#000] hover:border-blue-500 focus:border-[#000] hover:text-black py-3 px-4 rounded-md w-[24%]' >
                            <div>
                                <Image
